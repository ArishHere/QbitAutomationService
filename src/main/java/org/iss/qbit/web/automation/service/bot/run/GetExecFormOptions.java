...
package org.iss.qbit.web.automation.service.bot.run;
...
@RestController
@RequestMapping("/v2/exec/{robotName}/options/{paramName}")
@Slf4j
public class GetExecFormOptions
{
	...
	public String getParamValues(@PathVariable String robotName, @PathVariable String paramName, @RequestBody String jsonS) throws JSONException
	{
		...
		catch (Exception e)
		{
			// e.printStackTrace();
			status = false;
			GetExecFormOptions.log.error("Error preparing execution request.", e);
		}
		...
		{
			GetExecFormOptions.log.error("Error inserting values in json", e1);
			try
			{
			...
			{
				GetExecFormOptions.log.error("Error inserting error meaasge in json", e2);
			}
		}

		GetExecFormOptions.log.trace("{}", jsonResponse);

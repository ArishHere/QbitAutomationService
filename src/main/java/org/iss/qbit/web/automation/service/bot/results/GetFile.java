...
//	}
	@RequestMapping(value = "/v2/get/{robotName}/file/{sid}", method = RequestMethod.GET)
	//    @GET
	//    @Path("/File")
...
//			out.println("<body><h1>Error=" + e.getMessage() + "</h1></body></html>");
	...
	/*@RequestMapping("/v2/get/helpGuide")
    public ResponseEntity<InputStreamResource> getPDF() throws IOException {
        ClassPathResource pdf = new ClassPathResource("webui_userguide.pdf");
 
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.parseMediaType("application/pdf"));
        headers.add("content-disposition", "inline;filename=ISS_Webui - a quick overview.pdf");
        return ResponseEntity
                .ok()
                .headers(headers)
